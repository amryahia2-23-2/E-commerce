...
import { BlockCategory, BLOG_OTHER_CATEGORIES_QUERY_RESULT , BLOG_CATEGORIES_QUERY_RESULT} from "@/sanity.types"
...
async function BlogLeftSide({slug} : {slug: string}) {
    const blogCategories: BLOG_CATEGORIES_QUERY_RESULT | null = await getBlogCategories();
    const blogs: BLOG_OTHER_CATEGORIES_QUERY_RESULT | null= await getBlogOtherCategories(slug , 5);
    ...
    const allCategories = blogCategories?.flatMap(
        ...
        const uniqueCategories: BlockCategory[] = Array.from(
